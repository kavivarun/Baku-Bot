...
try:
    ...
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "NHentai-API==0.0.16"])
    from NHentai.nhentai import NHentai
    ...
    async def sauce(self, ctx, *, hid=""):
        if ctx.channel.is_nsfw():
            ...
            if hid == "":
                ...
                Doujin = nhentai.get_doujin(id=popular.doujins[random.randint(
                    0, popular.total_doujins - 1)].id)
                ...
                try:
                    ...
                    Doujin = nhentai.get_doujin(id=hsearch.doujins[
                        random.randint(0, (size - 1) // total)].id)
                except:
                    await ctx.send(
                        "Enter a valid id or search string else leave search field empty "
                    )
            ...
            if Doujin:
                embed = discord.Embed(
                    title=Doujin.title,
                    url='https://nhentai.net/g/{0}/'.format(Doujin.id),
                    description=str(Doujin.total_pages) + " Pages",
                    color=discord.Color.red())
                embed.add_field(name="**Id**\n",
                                value=Doujin.id + "\n",
                                inline=True)
                embed.add_field(name="**Tags**",
                                value="`" + "` `".join(Doujin.tags) + '`',
                                inline=False)
                embed.set_footer(text='Artists: {0}'.format(", ".join(
                    Doujin.artists) if len(Doujin.artists) > 0 else "Unknown"))
                ...
                try:
                    reaction, user = await self.client.wait_for(
                        ...
                        (reaction.emoji == '📖' or reaction.emoji == '❌'
                         ) and user == ctx.author and reaction.message.id ==
                        message.id,
                        timeout=30.0)
                except:
                    pass
                    ...
                    if reaction.emoji == "❌":
                        ...
                        await message.delete()
                        await ctx.invoke(self.client.get_command('read'),
                                         hid=str(Doujin.id))
        else:
                ...
                while loop:
                    try:
                        reaction, user = await self.client.wait_for(
                            'reaction_add',
                            check=lambda reaction, user:
                            (reaction.emoji == '👉' or reaction.emoji == '👈' or
                             reaction.emoji == '❌') and user == ctx.author and
